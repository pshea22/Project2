...
var actorChars = {
  "@": Player,
  "o": Coin,
  "l": Sink, // A coin will wobble up and down
  "s": Zoom,
  "=": Lava, "|": Lava, "v": Lava
};
...
Sink.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
var wobbleSpeed = 9, wobbleDist = 0.15;
...
Player.prototype.act = function(step, level, keys) {
  ...
  // Losing animation
  if (level.status == "lost" || level.status == "losttube") {
    this.pos.y += step;
    this.size.y -= step;
  }
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin" || type == "zoom" || type == "sink") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
    if (!this.actors.some(function(actor) {
          return actor.type == "zoom";
        }))
        {
    ...
    if (!this.actors.some(function(actor) {
           return actor.type == "sink";
         })) {
      this.status = "losttube";
      this.finishDelay = 1;
    }
  }
};
...
function runGame(plans, Display) {
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost" || status == "losttube")
        startLevel(n);
